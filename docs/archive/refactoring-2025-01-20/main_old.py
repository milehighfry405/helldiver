...
# Suppress Neo4j driver warnings (verbose property/label notifications)
...
parser.add_argument('--test', action='store_true', help='Test mode: 30-second research (uses haiku, minimal tokens)')
args = parser.parse_args()

# Set test mode flag (used by worker prompts)
TEST_MODE = args.test
...
class ResearchSession:
    ...
    def __init__(self):
        ...
        self.narrative = ""
        self.refinement_log = []  # Full refinement log (all conversation turns)
        self.pending_refinement = []  # Conversation since last research (cleared after each research)
...
def run_research_phase(session: ResearchSession, research_dir: str = None) -> str:
    ...
    print_status("SYNTHESIS", "Synthesizing narrative with Sonnet 4.5...")
    ...
    if episode_result and episode_result.get("status") == "success":
        ...
        if episode_names:
            representative_id = f"{session.episode_name} ({episode_count} episodes: research + context)"
            if is_deep_research:
                session.deep_episode_ids.append(representative_id)
                print_status("SUCCESS", f"Deep research: {episode_count} episodes committed (research + context)")
            else:
                session.initial_episode_id = representative_id
                print_status("SUCCESS", f"Initial research: {episode_count} episodes committed (research + context)")
    else:
        print_status("WARNING", "Graph write failed, but continuing")

    # Distill refinement context for this research (will be committed to graph as Episode 5)
    # This happens BEFORE saving so we can save the distilled version to file
    distilled_context = None
    if not is_deep_research and session.tasking_context:
        # Initial research: distill tasking conversation
        formatted_for_distill = []
        pending_assistant_msg = None

        for msg in session.tasking_context.get("conversation_history", []):
            role = msg.get("role", "")
            content = msg.get("content", "")

            if role == "assistant":
                # Store assistant message, waiting for next user message
                pending_assistant_msg = content
            elif role == "user":
                # Create turn with this user message and any pending assistant message
                formatted_for_distill.append({
                    "user_input": content,
                    "assistant_response": pending_assistant_msg or "",
                    "timestamp": datetime.now().isoformat()
                })
                pending_assistant_msg = None

        if formatted_for_distill:
            print_status("DISTILLING", "Extracting gold from tasking conversation...")
            distilled_context = distill_refinement_context(formatted_for_distill)
    elif is_deep_research and session.pending_refinement:
        # Deep research: distill pending refinement
        print_status("DISTILLING", "Extracting gold from refinement conversation...")
        distilled_context = distill_refinement_context(session.pending_refinement)

    # Save refinement context that led to this research (both full and distilled)
    # For initial research: use tasking_context + pending_refinement (should be empty)
    # For deep research: use tasking_context + pending_refinement (conversation since last research)
    save_refinement_context_to_research(
        research_dir=research_dir,
        tasking_context=session.tasking_context if not is_deep_research else {},
        refinement_log=session.pending_refinement,
        distilled=distilled_context
    )

    # Clear pending refinement (already saved to this research folder)
    session.pending_refinement = []
...
Respond with ONLY the intent word."""
        ...
        # Handle intents
        if "EXIT" in intent:
            # Check if there's uncommitted refinement conversation
            if session.pending_refinement:
                print("\nYou have refinement conversation that hasn't been committed to the graph.")
                print(f"({len(session.pending_refinement)} conversation turns with insights/aha moments)")
                commit_prompt = input("\nWould you like to commit this to the knowledge graph before exiting? ").strip()

                commit_check = f"""User said: "{commit_prompt}"

Are they agreeing to commit?

Respond with ONLY:
- YES if agreeing
- NO if declining"""

                commit_response = anthropic_client.messages.create(
                    model="claude-sonnet-4-5",
                    max_tokens=5,
                    temperature=0,
                    messages=[{"role": "user", "content": commit_check}]
                )

                commit_intent = ""
                for block in commit_response.content:
                    if block.type == "text":
                        commit_intent = block.text.strip().upper()

                if "YES" in commit_intent:
...
Be conversational, insightful, and help them think deeper."""
        ...
        # Log refinement (both full log and pending since last research)
        turn_data = {
        ...
        session.refinement_log.append(turn_data)
        session.pending_refinement.append(turn_data)
...
{findings}"""
    ...
    # COMMIT REFINEMENT CONTEXT AS 5TH EPISODE (ALWAYS, NO EXCEPTIONS)
    # This is the conversation that LED TO this research being executed
    # Extract the gold from tasking/refinement conversation and commit it

    # Determine which refinement context to use
    if research_type == "initial":
        # Initial research: use tasking context
        refinement_source = session.tasking_context.get("conversation_history", [])
        refinement_type = "Tasking Conversation"
    else:
        # Deep research: use pending refinement (conversation since last research)
        refinement_source = session.pending_refinement
        refinement_type = "Refinement Conversation"

    if refinement_source:
        # Distill refinement context to extract gold/signal
        print_status("EXTRACTING", "Distilling refinement context for graph commit...")

        if research_type == "initial" and "conversation_history" in session.tasking_context:
            # Format tasking conversation for distillation
            conversation_text = "\n\n".join([
                f"{msg.get('role', 'unknown').upper()}: {msg.get('content', '')}"
                for msg in session.tasking_context["conversation_history"]
            ])
        else:
            # Format refinement conversation for distillation
            conversation_text = "\n\n".join([
                f"USER: {turn['user_input']}\n\nASSISTANT: {turn['assistant_response']}"
                for turn in refinement_source
            ])

        # Distill to extract mental models, key questions, strategic context
        # For initial research: convert tasking context to refinement_log format for distillation
        if research_type == "initial":
            # Convert tasking conversation_history to refinement_log format
            formatted_for_distill = []
            pending_assistant_msg = None
            ...
            distilled = distill_refinement_context(formatted_for_distill)
        else:
            # Deep research: use pending_refinement directly
            distilled = distill_refinement_context(refinement_source)

        # Episode name for refinement context
        refinement_episode_name = f"{session_name} - {refinement_type} (Context)"

        # Episode body: Distilled gold from conversation
...
ORIGINAL CONVERSATION LENGTH: {len(refinement_source)} turns
...
research findings when interpreting results."""
        ...
        source_description_refinement = f"{research_type.title()} Research Context | Session: {session_name} | {timestamp}"

        # Commit refinement episode with retry logic
        max_retries = 3
        retry_delay = 2
        ...
        for attempt in range(max_retries):
            try:
                refinement_result = await graphiti_client.commit_episode(
                    agent_id="helldiver",
                    original_query=refinement_episode_name,
                    tasking_context={
                        "type": f"{research_type}_refinement_context",
                        "worker": refinement_type,
                        "session": session_name,
                        "parent_episode": parent_episode_id if parent_episode_id else "root",
                        "group_id": group_id,
                        "source_description": source_description_refinement,
                        "weighting": "refinement_context_weighted_higher_than_research"
                    },
                    findings_narrative=refinement_body,
                    user_context=source_description_refinement
                )

                if refinement_result and refinement_result.get("status") == "success":
                    episode_results.append(refinement_result.get("episode_name", refinement_episode_name))
                    print_status("EPISODE", f"✓ {refinement_episode_name} (CONTEXT - WEIGHTED HIGHER)")
                    break
                elif refinement_result and "rate limit" in refinement_result.get("error", "").lower():
                    if attempt < max_retries - 1:
                        wait_time = retry_delay * (attempt + 1)
                        print_status("RETRY", f"Rate limit hit, waiting {wait_time}s before retry {attempt + 2}/{max_retries}")
                        time_module.sleep(wait_time)
                    ...
                    print_status("ERROR", f"Failed to commit {refinement_episode_name}: {refinement_result.get('error', 'Unknown error')}")
                    break
            except Exception as e:
                if attempt < max_retries - 1 and "rate limit" in str(e).lower():
                    wait_time = retry_delay * (attempt + 1)
    ...
    # Return summary (should be 5 episodes: 3 workers + critical + refinement context)
    return {
        "status": "success" if episode_results else "error",
        "episode_names": episode_results,
        "episode_count": len(episode_results),
        "message": f"Committed {len(episode_results)} episodes (research + context)"
    }
...
def synthesize_narrative(worker_results: dict, critical_analysis: str, research_query: str, tasking_context: str) -> str:
    """Synthesize worker findings into dense narrative using Sonnet 4.5"""
...
def save_refinement_context_to_research(research_dir: str, tasking_context: dict, refinement_log: list, distilled: str = None):
    """Save refinement context to specific research directory

    This captures the conversation that LED UP TO this research being triggered.
    Called immediately after research completes.

    Args:
        research_dir: Path to the research folder (initial or deep)
        tasking_context: Tasking conversation (for initial research) or prior refinement (for deep research)
        refinement_log: Refinement conversation that led to this research
        distilled: Distilled/extracted gold from conversation (what goes to graph)
    """
    if not tasking_context and not refinement_log:
        return

    # Save human-readable full conversation (for audit trail)
    readable_file = os.path.join(research_dir, "refinement_context.txt")
    with open(readable_file, 'w', encoding='utf-8') as f:
        ...
        # Include tasking conversation (for initial research)
        if tasking_context and "conversation_history" in tasking_context:
            f.write("TASKING PHASE (Defining What to Research)\n")
            f.write("="*80 + "\n\n")

            for i, msg in enumerate(tasking_context["conversation_history"], 1):
        ...
        # Include refinement conversation (for deep research)
        if refinement_log:
            if tasking_context and "conversation_history" in tasking_context:
                f.write("\n\nREFINEMENT PHASE (Conversation Before Triggering Research)\n")
                f.write("="*80 + "\n\n")
            else:
                f.write("REFINEMENT CONVERSATION (Led to This Research)\n")
                f.write("="*80 + "\n\n")
    ...
    # Save distilled version (what gets committed to graph)
    if distilled:
        distilled_file = os.path.join(research_dir, "refinement_distilled.txt")
        with open(distilled_file, 'w', encoding='utf-8') as f:
...
def save_refinement_context(session: ResearchSession):
    """DEPRECATED: Old function that saved to session root. Kept for compatibility with old commit_to_graph_phase."""
...
async def retroactive_commit_research(session: ResearchSession, session_dir: str):
    ...
    if not session.initial_episode_id:
        ...
        if os.path.exists(narrative_file):
            ...
            # Load refinement distilled (for Episode 5 commit)
            refinement_distilled_file = os.path.join(initial_dir, "refinement_distilled.txt")
            refinement_distilled = ""
            if os.path.exists(refinement_distilled_file):
                with open(refinement_distilled_file, 'r', encoding='utf-8') as f:
            ...
            # For migration: populate tasking_context with a fake conversation so Episode 5 gets committed
            # The distilled content already exists in refinement_distilled.txt
            old_tasking = session.tasking_context
            if refinement_distilled and not session.tasking_context.get("conversation_history"):
                # Create a minimal conversation history that will trigger Episode 5 commit
                session.tasking_context = {
                    "conversation_history": [
                        {"role": "user", "content": session.query},
                        {"role": "assistant", "content": f"[Refinement context loaded from file]\n\n{refinement_distilled}"}
                    ]
                }
            ...
            # Restore original tasking context
            session.tasking_context = old_tasking
    ...
    for i in range(1, session.deep_research_count + 1):
        if i > len(session.deep_episode_ids):
            ...
            if deep_dirs:
                ...
                # Load refinement distilled (for Episode 5 commit)
                refinement_distilled_file = os.path.join(deep_dir, "refinement_distilled.txt")
                refinement_distilled = ""
                if os.path.exists(refinement_distilled_file):
                    with open(refinement_distilled_file, 'r', encoding='utf-8') as f:
                        refinement_distilled = f.read()
                ...
                # For migration: populate pending_refinement so Episode 5 gets committed
                old_pending = session.pending_refinement
                if refinement_distilled and not session.pending_refinement:
                    # Create a minimal refinement log that will trigger Episode 5 commit
                    session.pending_refinement = [
                        {
                            "user_input": topic,
                            "assistant_response": f"[Refinement context loaded from file]\n\n{refinement_distilled}",
                            "timestamp": datetime.now().isoformat()
                        }
                    ]
                ...
                # Restore original pending_refinement
                session.pending_refinement = old_pending
                session.query = old_query
...
def main():
    ...
    if args.test:
        print_status("TEST MODE", "Fast 30-second research (Haiku, 500 tokens, no web search)")
